<template>
  <div>
    ...
    <!--  表格开始    -->
    <el-table
      :data="demandForm.slice((pageNo - 1) * size, pageNo * size)"
      border
      style="width: 100%; margin-top: 30px"
    >
      <el-table-column
        prop="demandIntention.demandArea"
        label="意向区域"
        min-width="100"
      >
      </el-table-column>
      <el-table-column
        prop="demandIntention.villageName"
        label="意向小区"
        min-width="100"
      >
      </el-table-column>
      ...
      <el-table-column
        prop="rentalIntention.rentalName"
        label="客户需求"
        min-width="120"
      >
      </el-table-column>
      <el-table-column
        prop="demandIntention.minPrice"
        label="最低价格"
        min-width="160"
      >
      </el-table-column>
      <el-table-column
        prop="demandIntention.maxPrice"
        label="最高价格"
        min-width="160"
      >
      </el-table-column>
      <el-table-column
        prop="demandIntention.minArea"
        label="最低面积"
        min-width="160"
      >
      </el-table-column>
      <el-table-column
        prop="demandIntention.maxArea"
        label="最大面积"
        min-width="160"
      >
      </el-table-column>
      <el-table-column label="操作" fixed="right" width="300">
        <template v-slot:default="r">
          <el-button @click="(fangDialog = true), po(r.row)"
            >提供二手房房源</el-button
          >
          <el-button @click="(xinDog = true), po(r.row)"
            >提供新房房源</el-button
          >
    ...
    <el-dialog v-model="fangDialog" title="选择房源">
      <el-table
        :data="ershoufan.slice((pageNo2 - 1) * size2, pageNo2 * size2)"
        border
      ...
      >
        ...
        <el-table-column prop="secondaryTitle" label="出售标题" width="120">
        </el-table-column>
            ...
            <el-row>
              ...
              <el-col :span="10">
                <span style="font-size: 18px; font-weight: bold">{{
                  r.row.secondaryVillage +
                  r.row.secondaryBuilding +
                  r.row.secondaryUnit
                }}</span>
                <br />
                <span>{{
                  r.row.secondaryModel +
                  " " +
                  r.row.secondaryToward +
                  " " +
                  r.row.secondaryType
                }}</span>
                <br />
                <span style="font-size: 25px; color: #59ff00"
                  >{{ r.row.secondaryPrice }}万</span
                >
              </el-col>
            </el-row>
          </template>
    ...
    <el-dialog v-model="selectIs" title="查看详情" width="700px">
      ...
      公客池需求
      <p>
        求购意向:{{ customers.rentalIntention.rentalName }},意向区域:{{
          customers.demandIntention.demandArea
        }},户型:<template
          v-for="item in customers.demandIntention.demandModelKeys"
          :key="item.modelId"
          ><label>{{ item.modelName }}</label></template
        >
      </p>
      <p>
        价格区间:{{ customers.demandIntention.minPrice }}--{{
          customers.demandIntention.maxPrice
        }}万,面积区间{{ customers.demandIntention.minArea }}--{{
          customers.demandIntention.maxArea
        }}m²
      </p>
      <p>客户信息</p>
      <p>
        客户称呼:{{ customers.cusName }},性别:{{ customers.cusSex }},电话:{{
          customers.cusPhone
        }}
      </p>
      <label>推荐房源</label>
      ...
      <el-tabs v-model="activeName">
        <el-tab-pane label="新房" name="first" :key="'first'">
          <el-table
            :data="recommended.slice((pageNo3 - 1) * size3, pageNo3 * size3)"
            border
            style="width: 100%"
          >
            ...
            <el-table-column
              prop="newHouse.houseAddress"
              label="小区地址"
            ></el-table-column>
            <el-table-column
              prop="newHouse.housePosition"
              label="小区位置"
            ></el-table-column>
            <el-table-column
              prop="newHouse.houseState"
              label="销售状态"
            ></el-table-column>
            <el-table-column
              prop="newHouse.houseAddress"
            ...
            ></el-table-column>
            <el-table-column
              prop="newHouse.houseDeveloper"
              label="开发商名称"
            ></el-table-column>
            <el-table-column
              prop="newHouse.houseNumber"
              label="楼盘编号"
            ></el-table-column>
          </el-table>
          <!-- 分页开始   -->
        ...
        <el-tab-pane label="二手房" name="second" :key="'second'">
          <el-table
            :data="recommendeder.slice((pageNo5 - 1) * size5, pageNo5 * size5)"
            border
            style="width: 100%"
          >
            <el-table-column prop="secondary.secondaryId" label="序号">
            </el-table-column>
            <el-table-column
              prop="secondary.secondaryTitle"
              label="出售标题"
              width="120"
            >
            </el-table-column>
            <el-table-column label="房源信息" width="450">
              <template v-slot:default="r">
                <el-row>
                  <el-col :span="12">
                    <img
                      :src="r.row.secondary.secondaryPicture"
                      width="200"
                      height="100"
                    />
                  </el-col>
                  <el-col :span="10">
                    <span style="font-size: 18px; font-weight: bold">{{
                      r.row.secondary.secondaryVillage +
                      r.row.secondary.secondaryBuilding +
                      r.row.secondaryUnit
                    }}</span>
                    <br />
                    <span>{{
                      r.row.secondary.secondaryModel +
                      " " +
                      r.row.secondary.secondaryToward +
                      " " +
                      r.row.secondary.secondaryType
                    }}</span>
                    <br />
                    <span style="font-size: 25px; color: #59ff00"
                      >{{ r.row.secondary.secondaryPrice }}万</span
                    >
                  </el-col>
                </el-row>
              </template>
          ...
          <div style="text-align: center; margin-top: 10px">
            <el-pagination
              ...
              :current-page="pageNo5"
              :page-sizes="[5, 10, 15, 20]"
              :page-size="size5"
              layout="total, sizes, prev, pager, next, jumper"
              :total="recommendeder.length"
            >
            </el-pagination>
          </div>
        </el-tab-pane>
    ...
    <el-dialog v-model="xinDog" title="选择房源">
      <el-table
        :data="chamber.slice((pageNo4 - 1) * size4, pageNo4 * size4)"
        border
        @selection-change="xuan2"
        style="width: 100%"
      >
        <el-table-column fixed="left" type="selection" width="55">
        </el-table-column>
